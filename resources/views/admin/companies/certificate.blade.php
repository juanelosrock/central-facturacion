<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">
            Paso 3: Configurar certificado — {{ $company->business_name }}
        </h2>
    </x-slot>	
    <div class="py-12 max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        @if (session('api_debug'))
            ...
            <div class="mb-6 bg-yellow-50 border-2 border-yellow-400 p-6 shadow rounded">
                ...
                <div class="mb-3">
                    <p class="font-semibold mb-1">📤 JSON enviado (certificate truncado por tamaño):</p>
                    @php
                        $displayPayload = $debug['payload'];
                        if (isset($displayPayload['certificate'])) {
                            $len = strlen($displayPayload['certificate']);
                            $displayPayload['certificate'] = substr($displayPayload['certificate'], 0, 80) . '... [' . $len . ' chars]';
                        }
                    @endphp
                    <pre class="bg-gray-900 text-blue-200 p-3 rounded text-xs overflow-auto max-h-96">{{ json_encode($displayPayload, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                </div>
        ...
        <div class="bg-white p-6 shadow rounded">
            <form method="POST" action="{{ route('admin.companies.certificate.update', $company) }}" enctype="multipart/form-data">
                @csrf @method('PUT')
                ...
                <h3 class="font-semibold text-lg mb-3 border-b pb-1">Certificado DIAN (.p12)</h3>

                <div class="mb-4">
                    <label class="block text-sm font-semibold mb-1">Archivo del certificado</label>
                    <input type="file" name="certificate_file" accept=".p12,.pfx"
                           class="w-full border rounded p-2 bg-white" required>
                    <p class="text-xs text-gray-500 mt-1">
                        Subí el archivo <code>.p12</code> (o <code>.pfx</code>). El sistema lo convierte a base64 automáticamente antes de enviarlo al API.
                    </p>
                    @if ($certificate && $certificate->name)
                        <p class="text-xs text-blue-600 mt-1">
                            Archivo actual: <strong>{{ $certificate->name }}</strong>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="password"
                           value="{{ old('password', $certificate->password ?? '') }}"
        ...
        {{-- Detalles del certificado ya configurado --}}
        @if ($certificate && $certificate->last_synced_at)
            <div class="bg-white p-6 shadow rounded mt-6">
                <h3 class="font-semibold mb-3">✅ Certificado configurado</h3>
                <p class="text-sm text-gray-600 mb-3">
                    Última sincronización: {{ $certificate->last_synced_at->diffForHumans() }}
                </p>

                <dl class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    <div>
                        <dt class="font-semibold">ID interno (Qimera)</dt>
                        <dd>{{ $certificate->api_certificate_id ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="font-semibold">Nombre del archivo</dt>
                        <dd class="font-mono">{{ $certificate->name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="font-semibold">Vencimiento</dt>
                        <dd>
                            {{ $certificate->expiration_date?->format('d/m/Y H:i') ?? '—' }}
                            @if ($certificate->expiration_date)
                                <span class="text-xs text-gray-500">
                                    ({{ $certificate->expiration_date->diffForHumans() }})
                                </span>
                                @if ($certificate->expiration_date->isPast())
                                    <span class="ml-2 px-2 py-0.5 bg-red-200 text-red-800 rounded text-xs">VENCIDO</span>
                                @elseif ($certificate->expiration_date->diffInDays(now()) < 30)
                                    <span class="ml-2 px-2 py-0.5 bg-yellow-200 text-yellow-800 rounded text-xs">POR VENCER</span>
                                @endif
                            @endif
                        </dd>
                    </div>
                    <div>
                        <dt class="font-semibold">Tamaño del certificado (base64)</dt>
                        <dd>{{ number_format(strlen($certificate->certificate) / 1024, 1) }} KB</dd>
                    </div>
                </dl>
            </div>
        @endif
